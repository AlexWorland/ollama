...
## How can I limit disk usage for evicted K/V cache?

When the K/V cache exceeds the in-memory threshold, the oldest cached prefixes
can be evicted to disk (under `<OLLAMA_MODELS>/../cache/kv/<digest>/`) so they
can be restored on subsequent requests without recomputation. This avoids
re-running prompt prefill for recurring conversation prefixes across restarts.

- `OLLAMA_KV_CACHE_DISK_MAX` - Maximum disk usage (bytes) for evicted K/V cache
  entries per model. Default is `0` (unlimited). When exceeded, the oldest
  cache files are deleted.

<Note>
  Disk-backed K/V cache is currently implemented for the MLX runner
  (Apple Silicon). Other runners ignore this variable.
</Note>
