...
// CollectArrays serializes the conv/delta state. offset is recovered from
// the trie node's toOffset at load time; recurrent restore only succeeds
// when target == offset, which naturally matches that value.
...
func (c *RecurrentCache) Restore(seqID int, snapshot Snapshot, target int) bool {
	...
	// Recurrent snapshots encode cumulative state up to exactly snap.offset —
	// target must match; rewinding would leave stale state and advancing
	// isn't possible without feeding tokens.
