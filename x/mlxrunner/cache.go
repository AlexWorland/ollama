...
type kvCache struct {
	...
	// Persistence state (cache-persisitance branch additions).
	cacheDir    string // disk dir for content-addressed snapshot files; "" disables persistence
	modelDigest string // model identity bound into filename hash
	diskBytes   int64  // total bytes on disk across the trie
	writer      *diskWriter
}
...
// activeNodeSet returns the set of all trie nodes referenced by any active path.
